...
class AccountTaxAng(models.Model):
    ...
    post_retention_payment = fields.Boolean(
        string="Reter na DLI, ao invés do Recibo",
        help="Se marcado, a retenção será lançada apenas no momento do pagamento ao estado, em vez de no momento da emissão da fatura."
    )
        ...
        for tax in taxes:
            ...
            for repartition_line, line_amount in zip(tax_repartition_lines, repartition_line_amounts):
                ...
                if not (tax.dont_affect_invoice or tax.margin_affect) and (self.env.context.get("default_move_type") in \
                    ...
                    'payment_with_withholding') == 'with' and self.env.context.get('payment_difference_handling') != 'open':
                    taxes_vals.append({
                        'id': tax.id,
