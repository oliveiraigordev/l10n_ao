...
class WizardHRRemunerationMap(models.TransientModel):
    _name = 'wizard.hr.cost.analysis.map'
    _description = 'Print Cost Analysis Map'
            ...
            data = {
                'form': self.read(
                    [
                        ...
                        'hr_payslip_run_id',
                        'start_date',
                        'end_date', 'contract_type_id'])[
                    0]}
            self.url_download = self.env['report.cost_analysis_map'].cost_analysis_map_report(self, data=data)
