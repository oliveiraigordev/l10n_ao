...
class WizardIRTMap(models.TransientModel):
    _name = 'wizard.hr.irt.map'
    ...
    def print_report_xlsx(self):
        if self.env.company.country_id.code == "AO":
            data = {'form': self.read(
                ['slip_filter_by', 'hr_payslip_run_id', 'start_date', 'end_date', 'contract_type_id'])[0]}
            self.url_download = self.env['report.l10n_ao_hr.irt_map'].irt_map_xlsx_report(self, data)
    ...
    def print_report(self, report_file=False):
        if self.env.company.country_id.code == "AO":
            data = {'form': self.read(
                ['slip_filter_by', 'hr_payslip_run_id', 'start_date', 'end_date', 'contract_type_id'])[0]}
            return self.env.ref('l10n_ao_hr.action_irt_map').report_action(self, data)
